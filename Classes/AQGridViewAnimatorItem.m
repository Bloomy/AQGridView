...
{
    if ([o isKindOfClass:[self class]] == NO) return (NO);
...
- (NSComparisonResult)compare:(id)obj
{
    if ([obj isKindOfClass:[self class]] == NO) {
        if ( (void *)objc_unretainedPointer(self) < (void *)objc_unretainedPointer(obj) ) return (NSOrderedAscending);
        
        if ( (void *)objc_unretainedPointer(self) > (void *)objc_unretainedPointer(obj) ) return (NSOrderedDescending);
    ...
    if (self.index < item.index) return (NSOrderedAscending);
    
    if (self.index > item.index) return (NSOrderedDescending);
